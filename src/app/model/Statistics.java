...
public class Statistics {
    ...
    public static HashMap<String,Double> getDataForChartOnIncomeRecords(){
        return getDataForChartOnRecords(true);
    }

    public static HashMap<String,Double> getDataForChartOnExpRecords(){
        return getDataForChartOnRecords(false);
    }

    private static HashMap<String, Double> getDataForChartOnRecords(boolean income) {
        ...
        for (Transaction t : transactions) {
            if ((income && t.getAmount() > 0) || (!income && t.getAmount() < 0)) {
                String key = t.getRecord().getTitle();
                double sum = 0;
